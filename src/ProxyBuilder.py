...
def GetAllConnectionIn(obj, NextFunc, Filter = None):
    ...
    while nexts:
        ...
        if nexts:
            nexts = [x for x in nexts if x not in AllFound] #if we have not found it here, then its added
...
class BuildProxy:
    ...
    def BuildProxyForSelectedMesh(self):
        ...
        print(f"found mesh: {self.model}, skin: {self.skin}, joints: {self.jnts}")
...
class BuildProxyWidget(QWidget):
    def __init__(self):
        ...
        buildBtn = QPushButton("BuildProxy")
